// This program computes the color and index of each node 
// using the landscape algorithm. 
// NOTICE: before compiling the program, set USERNUM to be larger 
// than the actual user number. 
...
const unsigned int USERNUM = 80000000;
//map<unsigned int, unsigned int> uimap;

unsigned int cnt = 0;

User* user = new User[USERNUM];
Node* orderlist = new Node[USERNUM];
...
void output(string filename)
{
  ...
  outfile<<setw(12)<<cnt<<setw(12)<<LEVEL<<endl;
  for (unsigned int i = 0; i < cnt; i++){
    outfile<<setw(12)<<iumap[i]<<setw(12)<<user[i].color<<setw(12)<<user[i].level<<endl;
